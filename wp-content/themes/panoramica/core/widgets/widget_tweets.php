...
class Cpotheme_Widget_TwitterStream extends WP_Widget{
	...
	function Cpotheme_Widget_TwitterStream(){
		$widget_ops = array('classname' => 'cpotheme_twitter_stream', 'description' => __('Displays the latest tweets for a specific Twitter user.', 'cpocore'));
	...
	function widget($args, $instance){
		...
		if(is_array($tweets) && count($tweets) > 0){
			$output = '<ul class="twitter_stream" id="twitter-stream-'.$widget_id.'">';
			...
			foreach ($tweets as $key => $tweet_contents){
				...
				$output .= '<span class="tweet_content">';
				$output .= $contents;
				$output .= '</span>';
				$output .= '<small class="tweet_date">';
